  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content">
        {studentId && selectedStudent ? <StudentView student={selectedStudent} sessions={selectedSessions} month={month} setMonth={setMonth} openLog={openLog} setModal={setModal} setStudent={setStudent} exportReport={exportReport} onBack={() => { setStudentId(null); setPage('students') }} /> : page === 'dashboard' ? <Dashboard month={month} setMonth={setMonth} students={activeStudents} sessions={sessions} currentSessions={currentSessions} totalHours={totalHours} openStudent={(id) => setStudentId(id)} onViewAll={() => { setStudentId(null); setPage('students') }} openLog={() => openLog()} exportReport={exportReport} /> : page === 'students' ? <StudentsView students={students} sessions={sessions} openStudent={(id) => setStudentId(id)} onAdd={() => setModal('student')} /> : <SessionsView sessions={visibleSessions} students={students} search={search} setSearch={setSearch} openLog={openLog} removeSession={removeSession} />}
      </div>
    </main>
...
function Dashboard({ month, setMonth, students, sessions, currentSessions, totalHours, openStudent, onViewAll, openLog, exportReport }: { month: Date; setMonth: (date: Date) => void; students: Student[]; sessions: Session[]; currentSessions: Session[]; totalHours: number; openStudent: (id: string) => void; onViewAll: () => void; openLog: () => void; exportReport: () => void }) {
  ...
  return <>
    ...
    <div className="section-head"><div><h2>Your students</h2><p>A little support goes a long way.</p></div><button className="text-link" onClick={onViewAll}>View all students <ArrowRight size={15} /></button></div>
